...
if tase == 1:
    ...
    kogus = int(input("Ülesandete arv:"))
    for i in range(1, kogus + 1):
        arv11 = randint(min_arv, max_arv)
        arv22 = randint(min_arv, max_arv)
        tehe = randint(0, tehe_arv)  # 1:1, 2:2, 3:3
        mark = tehed[tehe]
        print(f"{arv11}{mark}{arv22}=", end="")
        vas = eval(str(arv11) + mark + str(arv22))
        vastus = int(input())
        if vastus == vas:
            oige += 1

        else:
            print("Õige vastus on", eval(str(arv11) + mark + str(arv22)))

elif tase == 3:
    kogus = int(input("Ülesandete arv:"))
    for i in range(1, kogus + 1):
        arv111 = randint(min_arv, max_arv)
        arv222 = randint(min_arv, max_arv)
        tehe = randint(0, tehe_arv)  # 1:1, 2:2, 3:3
        mark = tehed[tehe]
        print(f"{arv111}{mark}{arv222}=", end="")
        vas = eval(str(arv111) + mark + str(arv222))
        vastus = int(input())
        if vastus == vas:
            oige += 1

        else:
            print("Õige vastus on", eval(str(arv111) + mark + str(arv222)))
